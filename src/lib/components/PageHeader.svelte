<script lang="ts">
	const sideQuestLogo =
		'https://raw.githubusercontent.com/valentinomettifogo/valmet-assets/main/img/side-quest-logo.jpg';
</script>

<header class="border-b border-neutral-200 pb-10 md:border-b-0 md:pb-0">
	<h1 class="text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl">
		Side Quest
	</h1>
	<p class="mt-3 text-neutral-600">
		Small projects, shipped over a weekend and pushed live anyway. No roadmap, no
		backlog — just whatever I felt like building next.
	</p>

	<img
		src={sideQuestLogo}
		alt="Side Quest"
		class="mt-6 w-full max-w-48 rounded-lg border border-neutral-200 shadow-sm"
	/>

	<p class="mt-8 text-sm text-neutral-400">
		Valentino Mettifo<span class="font-mono font-semibold text-[#00ADD8]">go</span>
	</p>
</header>
