<script lang="ts">
	...
	const field =
		'w-full border-3 border-line bg-transparent px-3 py-2 font-mono text-xs focus:outline-none';
	const label = 'block font-mono text-[11px] font-bold uppercase mb-1.5';
	const hint = 'mt-1.5 mb-0 font-mono text-[11px] opacity-60';
...
>
	...
	<div>
		<label class={label} for="webhookToken">Webhook token (optional)</label>
		<input
			id="webhookToken"
			name="webhookToken"
			type="password"
			class={field}
			autocomplete="off"
			placeholder={editing?.hasOwnToken ? 'set' : 'uses the global token'}
		/>
		<p class={hint}>Stored as a hash only. Revocable without touching other tenants.</p>
		{#if editing?.hasOwnToken}
			<label class={clear}>
				<input type="checkbox" name="clear_webhookToken" class="size-4" /> Fall back to the global token
			</label>
		{/if}
	</div>

	<div class="sm:col-span-2">
		<label class={label} for="chatWebhookUrl">Google Chat webhook (optional)</label>
		<input
			id="chatWebhookUrl"
			name="chatWebhookUrl"
			type="password"
			class={field}
			autocomplete="off"
			placeholder={editing?.hasOwnChatWebhook ? 'set' : 'blank = default space'}
		/>
		<p class={hint}>
			It carries key and token in the query string, so it never travels back to the
			browser: blank leaves it untouched.
		</p>
		{#if editing?.hasOwnChatWebhook}
			<label class={clear}>
				<input type="checkbox" name="clear_chatWebhookUrl" class="size-4" /> Fall back to the default space
			</label>
		{/if}
	</div>
	...
	<div class="flex flex-wrap items-center justify-end gap-3">
		{#if editing}
			...
			<button
				...
				formaction="?/delete"
				formnovalidate
				class="cursor-pointer border-3 border-red bg-transparent px-3 py-2 font-mono text-[11px] font-bold text-red uppercase hover:bg-red hover:text-bg"
			>
				Delete
			</button>
			<button
				type="button"
				onclick={onDone}
				class="cursor-pointer border-0 bg-transparent font-mono text-[11px] font-bold uppercase hover:text-red"
			>
				Cancel
			</button>
		{/if}
