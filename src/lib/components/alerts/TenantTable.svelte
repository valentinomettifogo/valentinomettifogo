...
<div class="flex items-center justify-between gap-3 border-b-3 border-line px-6 py-4">
	...
	<button
		...
		class="cursor-pointer border-3 border-line bg-transparent px-3 py-1.5 font-mono text-[11px] font-bold uppercase hover:bg-ink hover:text-bg disabled:cursor-not-allowed disabled:opacity-40"
	>
...
{:else}
	<div class="overflow-x-auto">
		<table class="w-full min-w-180 border-collapse text-left">
			<thead>
				<tr class="border-b-3 border-line font-mono text-[11px] uppercase">
			...
			<tbody>
				{#each tenants as tenant (tenant.id)}
					<tr class="border-b-3 border-line last:border-b-0" class:opacity-50={!tenant.active}>
						<td class="px-6 py-3">
							<span class="font-bold">{tenant.client}</span>
							{#if !tenant.active}
								<span class="ml-2 font-mono text-[11px] text-red uppercase">inactive</span>
							{/if}
							<div class="font-mono text-[11px] opacity-60">
								?cliente={tenant.slug}
								{#if tenant.hasOwnToken}· own token{/if}
								{#if tenant.hasOwnChatWebhook}· own chat space{/if}
							</div>
						</td>
						...
						<td class="px-6 py-3">
							<div class="flex justify-end gap-2">
								<button
									...
									class="cursor-pointer border-3 border-line bg-transparent px-2.5 py-1 font-mono text-[11px] font-bold uppercase hover:bg-ink hover:text-bg disabled:opacity-40"
								>
									Check
								</button>
								<button
									...
									class="cursor-pointer border-3 border-line bg-transparent px-2.5 py-1 font-mono text-[11px] font-bold uppercase hover:bg-ink hover:text-bg"
								>
									Edit
								</button>
							</div>
