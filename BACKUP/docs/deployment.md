...
2. Open the SQL editor and run [`supabase/schema.sql`](../supabase/schema.sql) in full.
   ...
   > **Upgrading an existing project:** this file also drops `chat_webhook_url` and
   > `webhook_token_hash` from `qlik_tenants` and drops `qlik_webhook_events` entirely —
   > irreversible against live data. Deploy the matching app code (see the rest of this
   > guide) *before* running the file against a project that already has data in those
   > columns/table, otherwise the still-deployed old code breaks. A brand-new project has
...
**Verify:** the table editor shows `qlik_tenants`, `app_users` and `app_settings`, all
with RLS enabled and zero policies. That is intentional: only the server touches them.

**Bootstrap the first admin.** There is no self-serve first-admin flow, by design. Sign
in once through the app — it will 403, because `app_users` is empty. Then, in the table
editor, insert one row into `app_users`: your email (lowercase), `role = 'admin'`, and
leave `user_id` blank — the app links it to your account automatically the next time you
sign in.
...
2. Authentication → Providers → **Email: disable it.**
   ...
   This matters. Access is granted per-email in `app_users`, and without this an
   attacker could simply sign up with a granted address via email/password instead of
   Google. The code also asserts `app_metadata.provider === 'google'` as a second lock,
   but both should be in place.
...
2. Add every variable from [`.env.example`](../.env.example):
   ...
   There is no env var for the Chat webhook or for who gets into `/alerts` anymore —
   those are set from `/alerts-settings` and the `app_users` table respectively (see
   step 1's bootstrap note and step 5).
...
**Verify:** the homepage loads and `/alerts` bounces you to `/login`. Sign in with the
account you bootstrapped as admin — you should land on `/alerts` with the empty tenant
table, and see both "Alerts" and "Alerts settings" in the navbar. Sign in with any other
Google account and you should get the 403 page **with a working sign-out button**.

---

## 5. Populate the tenants

Open `/alerts-settings` and add one tenant per `QLIK_KEY_<host>` Script Property in the
existing Apps Script project (Project Settings → Script Properties).
...
While you're there, set the Google Chat webhook once under **Notifications** — it applies
to every tenant, there is no per-tenant override anymore.

**Verify:** on `/alerts`, press **Check all**. Every row should report `Key valid`. A row
reporting `Key invalid or expired` means that key needs regenerating in the Qlik tenant —
worth knowing before you cut anything over.
...
Auth, in a browser (role from `app_users`):

| Role | Expected |
|---|---|
| `admin` | navbar shows Alerts + Alerts settings; both pages load |
| `analyzer` | navbar shows Alerts only; `/alerts-settings` 403s |
| `none` (or no row in `app_users`) | 403 on `/alerts` and `/alerts-settings`, no navbar visible anywhere |
