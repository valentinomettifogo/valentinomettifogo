...
3. **`/portal` panel** — Qlik tenant management behind Google sign-in, role `analytics`
   or higher. `/admin` (role `admin`) is still a placeholder.
...
`admin`, provisioned by a security-definer trigger and never self-editable (see
...
[`src/lib/server/authz.ts`](src/lib/server/authz.ts) ranks it. There is no `(protected)`
route group — each page guards itself in its own `+page.server.ts`, and so does every
form action.
...
└── routes/
    ...
    ├── portal/            # Qlik tenant panel (role: analytics)
    ├── admin/             # placeholder (role: admin)
